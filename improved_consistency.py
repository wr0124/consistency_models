...
viz = Visdom(env="consistency_test3")
...
class UNet(nn.Module):
    ...
    def from_pretrained(cls, pretrained_path: str) -> "UNet":
        ...
        state_dict = torch.load(
            os.path.join(pretrained_path, "model.ckpt"),
...
class LitImprovedConsistencyModel(LightningModule):
    ...
    def save_model_checkpoint(self):
        ...
        checkpoint_filename = f"step_{self.global_step}.ckpt"
...
def run_training(config: TrainingConfig) -> None:
    ...
    lit_icm = LitImprovedConsistencyModel(
        ...
        dataset_name=dataset_name,  # Pass dataset name here
        checkpoint_dir=checkpoint_dir,  # And checkpoint directory her
    )
    # Run training
